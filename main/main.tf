...
module "edge-nodes" {
  ...
  short_name = "${var.short_name}"
  availability_zones = "${module.vpc.availability_zones}"
  security_groups = ["${module.security-groups.edge_security_group}"]
  vswitch_id = "${module.vpc.vswitch_ids}"
}
...
module "slbs" {
  source = "../alicloud-slb-vpc"
  vswitch_id = "${module.vpc.vswitch_ids}"
  ssl_certificate_id = "${var.ssl_certificate_id}"
  control_instances = "${module.control-nodes.ecs_id_list}"
  worker_instances  = "${module.worker-nodes.ecs_id_list}"
}
